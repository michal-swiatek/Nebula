...
#include "Logging.h"
    ...
    #define NB_INTERNAL_ASSERT_WITH_MSG(type, check, msg) NB_INTERNAL_ASSERT_IMPL(type, check, "{1}:{2}: {0}", msg, std::filesystem::path(__FILE__).string(), __LINE__)
    #define NB_INTERNAL_ASSERT_NO_MSG(type, check) NB_INTERNAL_ASSERT_IMPL(type, check, "Assertion '{0}' failed at {1}:{2}", NB_STRINGIFY_MACRO(check), std::filesystem::path(__FILE__).string(), __LINE__)
